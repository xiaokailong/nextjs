...
本文档描述的 JSON 文件存储方案已被废弃。

**当前方案**：本地开发环境直接连接生产环境 API。

请参考 [DEV_WITH_PRODUCTION_API.md](./DEV_WITH_PRODUCTION_API.md) 了解当前的开发配置。
...
之前本项目使用 JSON 文件 (`src/data/db.json`) 作为本地开发环境的持久化存储。所有对面试题、学生、班级等数据的增删改查操作都会自动保存到此文件中。
